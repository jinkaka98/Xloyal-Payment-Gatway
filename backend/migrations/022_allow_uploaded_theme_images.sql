-- The theme editor stores a bounded raster logo as a data URI. Keep the
-- database publish constraint aligned with the HTTP validator so a valid
-- saved draft can be promoted to a checkout snapshot.
CREATE OR REPLACE FUNCTION payment_theme_safe_image_url(value TEXT)
RETURNS BOOLEAN
LANGUAGE sql
IMMUTABLE
AS $$
    SELECT value ~ '^https://[^[:space:]<>]+$'
        OR value ~ '^data:image/(png|jpeg|webp|gif);base64,[A-Za-z0-9+/=]+$';
$$;
...
BEGIN
    ...
    IF config ? 'branding' AND config->'branding' ? 'logo_url' AND NOT payment_theme_safe_image_url(config->'branding'->>'logo_url') THEN RETURN FALSE; END IF;
    IF config ? 'tenant_branding' AND config->'tenant_branding' ? 'logo_url' AND NOT payment_theme_safe_image_url(config->'tenant_branding'->>'logo_url') THEN RETURN FALSE; END IF;
    IF config ? 'tenant_branding' AND config->'tenant_branding' ? 'favicon_url' AND NOT payment_theme_safe_image_url(config->'tenant_branding'->>'favicon_url') THEN RETURN FALSE; END IF;
