...
''')
    ...
    next_page = page.locator("a[rel='next']:not(.disabled), button[aria-label='Next']:not([disabled]), .paginate_button.next:not(.disabled), .pagination .next:not(.disabled) a")
...
if start_date and end_date:
    ...
    for index in range(await inputs.count()):
        ...
        if "tanggal" in placeholder or "tanggal" in value.lower() or " - " in value:
            ...
            await candidate.fill("2026/07/14" + " - " + "2026/08/13")
    ...
    if await submit.count():
        ...
        expected_range = "2026/07/14" + " - " + "2026/08/13"
